...
class matrix
{
 public:
  ...
  // elements! First argument: number of rows Second argument: number of columns
  ...
  // Copy constructor to create a new matrix with the same elements as an
  // existing matrix
  matrix(const matrix &);
  ...
  // Overloaded operator = to assign the elements of one matrix to another
  // matrix
  ...
  // Member function used to obtain (but not modify) the number of rows in the
  // matrix
  size_t get_rows() const;

  // Member function used to obtain (but not modify) the number of columns in
  // the matrix
  size_t get_cols() const;

  // Overloaded operator () used to access matrix elements WITHOUT range
  // checking The indices start from 1: m(1, 2) would be the element at row 1,
  // column 2 First version: returns a reference, thus allows modification of
  // the element
  T &operator()(const size_t &, const size_t &);

  // Overloaded operator () used to access matrix elements WITHOUT range
  // checking The indices start from 1: m(1, 2) would be the element at row 1,
  // column 2 Second version: does not return a reference and declared as const,
 ...
 private:
  ...
  // A pointer to an array storing the elements of the matrix in flattened
  // (1-dimensional) form
  T *elements{nullptr};
...
{
  if (rows == 0) throw sayMessage{"Dimentions of matrix must be positive.\n"};
  smart.reset(new T[ rows * cols ]);
  elements = smart.get();
...
template <typename T>
matrix<T>::matrix(const size_t &input_rows, const size_t &input_cols,
                  const std::vector<T> &input_elements)
    : rows(input_rows), cols(input_cols)
{
  if (rows == 0 or cols == 0)
    throw sayMessage{"Dimentions of matrix must be positive.\n"};
  ...
  smart.reset(new T[ rows * cols ]);
  elements = smart.get();
  for (size_t i{0}; i < rows * cols; i++) elements[ i ] = input_elements[ i ];
}

// Full matrix initialized with initializer_list
template <typename T>
matrix<T>::matrix(const size_t &input_rows, const size_t &input_cols,
                  const std::initializer_list<T> &input_elements)
...
matrix<T>::matrix(const matrix<T> &m) : rows(m.rows), cols(m.cols)
{
  smart.reset(new T[ rows * cols ]);
  elements = smart.get();
  std::memcpy(elements, m.elements, rows * cols * sizeof(T));
}
