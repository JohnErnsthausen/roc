...
public:
    ...
    // Copy constructor to create a new vector with the same elements as an existing vector
    ...
    // Overloaded operator = to assign the elements of one vector to another vector
    ...
    // Member function used to obtain (but not modify) the size or length of the vector
    size_t get_size() const;

    // Overloaded operator () used to access vector elements WITHOUT range checking
    // The indices start from 1: v(2) would be the element at row 2
...
{
    if (size == 0 )
      throw sayMessage{"Size of vector must be positive.\n"};
...
vectorf<T>::vectorf(const vectorf<T> &v) : size(v.size)
{
    smart.reset(new T[size]);
    elements = smart.get();
    std::memcpy(elements, v.elements, size * sizeof(double));
}
