    ...
    TaylorSeriesAtZeroWithScalingTenthForComplexConjugatePolesAtPMOneFifthAlphaOne)
{
  ...
  EXPECT_THAT(err, DoubleNear(0.0, 100*DBL_EPSILON));
  EXPECT_THAT(rc, DoubleNear(sqrt(a * a + time * time), 10*DBL_EPSILON));
  EXPECT_THAT(order, DoubleNear(1.0, 300*DBL_EPSILON));
  EXPECT_THAT(coeffs.size(), Eq(30));
}
    ...
    TaylorSeriesAtZeroWithScalingThousandthForComplexConjugatePolesAtPMOneFifthAlphaThree)
{
  ...
  EXPECT_THAT(err, DoubleNear(0.0, 150*DBL_EPSILON));
  EXPECT_THAT(rc, DoubleNear(sqrt(a * a + time * time), 180*DBL_EPSILON));
  EXPECT_THAT(order, DoubleNear(3.1415, 2500*DBL_EPSILON));
  EXPECT_THAT(coeffs.size(), Eq(30));
}
    ...
    TaylorSeriesAtPT3WithScalingThousandthForComplexConjugatePolesAtPMOneFifthAlphaThree)
{
  ...
  EXPECT_THAT(err, DoubleNear(0.0, 2600*DBL_EPSILON));
