...
TEST_F(TestThatQRSIntegrationTest, WillComputeLeastSquarsSolution)
{
  ...
  EXPECT_THAT(qrs(m, n, a, lda, tau, y, x, &ier), Eq(0));
  EXPECT_THAT(ier, Eq(0));
  EXPECT_THAT(x(ipiv(1)),
              DOUBLE_NEAR_MULTIPLIER(-9.340358069665058e-01, 100.0));
  EXPECT_THAT(x(ipiv(2)), DOUBLE_NEAR_MULTIPLIER(8.847241983085994e-01, 100.0));

  // If Taylor coefficients TC(i) are unscaled and T(i) = (1/scale)^i TC(i),
  // then multiply Rc by scale in this computation on scaled TCs.
  //
  ...
  // Log10(L) = Log10(abs(T(i+1))/abs(T(i))) = Log10(abs(T(i+1))) -
  // Log10(abs(T(i)))
  //           = m*(i+1)+b - (m*i+b) = m
  // rc = 1/L = scale/pow(10, m).
  double Rc = 1.0 / pow(10, x(ipiv(2)));
  EXPECT_THAT(Rc, DOUBLE_NEAR_MULTIPLIER(1.303994626296309e-01,
                                         100.0));  // Should be exactly 1/8
}
