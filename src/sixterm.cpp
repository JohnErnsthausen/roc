...
//
// The method returns the absolute value of the penultimate equation to nUse parameter,
// which must be at least 4, evaluated at the solution to the least squares optimal solution.
//
// The computation is successful whenever the value return is acceptably small, otherwise
// the algorithm is said to detect that the coefficients do not resemble pair of complex
// conjugate poles.
//
...
#define W(i, j) W[ map(i, j) ]
  ...
  if( b(4) < 0 )
  {
    message = "Unconstrained optimization lead to Sqrt of negative number: " + to_string(b(4)) + " \n";
    throw sayMessage(message);
  }
  hOverRc = sqrt(b(4));
  ...
  if( isnan(rc) )
  {
    message = "The radius of convergence is infinity, which is highly unlikely\n";
    throw sayMessage(message);
  }
  ...
  if( isnan(cosTheta) )
  {
    message = "Unconstrained optimization lead to infinite CosTheta which is not in [-1, 1]\n";
    throw sayMessage(message);
  }   
  if( (cosTheta < -1.0) || (cosTheta > 1.0) )
  {
    message = "Unconstrained optimization lead to CosTheta [" + to_string(cosTheta) + "] not in [-1, 1]\n";
    throw sayMessage(message); 
  }
